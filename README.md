...
### ~Installing~ **Need actual bash/cmd install steps**Excluded from production
-TBH
...
#### Add/Update Account
Need more details on what the account is. Like is this the account that you AUTH into locally, or the 'account' used on the cloud to identify the user? What is jwt, where is userId initialized argument/come from? On the backend what actions does this call perform for the 'account'? does it generate the wallet for the account etc? 
```
...
@Raul Details on argument is needed
```
